...
+ (id)attributeValueWithArray:(NSArray *)arr emptyElementType:(int)et
{
	...
	else if( cnt < 65536 )
	{
		...
		{
			NSLog(@"XfirePacketAttributeValue: array element is not a valid class");
			return nil;
		}
		for( i = 1; i < cnt; i++ )
		{
			...
			if( objType != [XfirePacketAttributeValue typeIDForObject:arrObj] )
			{
				...
				NSLog(@"XfirePacketAttributeValue: incompatible types while scanning array");
				return nil;
			}
		}
...
- (id)initWithValue:(id)aVal typeID:(int)tid arrayType:(int)atid
{
	...
	if( self )
	{
		...
		//NSLog(@"XfirePacketAttributeValue -initWithValue:%@ typeID:%d arrayType:%d",aVal,tid,atid);
	}
	return self;
}
