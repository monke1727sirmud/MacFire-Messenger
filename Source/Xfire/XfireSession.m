...
- (id)initWithHost:(NSHost *)xfireHost port:(unsigned short)portNumber;
@end

NSString *kXfireVersionTooOldReason = @"Version too old";
NSString *kXfireInvalidPasswordReason = @"Invalid password";
NSString *kXfireNetworkErrorReason = @"An error occurred";

NSString *kXfireOtherSessionReason = @"Logged in on other PC";
NSString *kXfireServerHungUpReason = @"Xfire server hung up";
NSString *kXfireServerStoppedRespondingReason = @"Xfire server stopped responding";
...
- (void)connect
{
	...
	_loginConnection = [XfireConnection loginConnectionToHost:_xfireHost port:_xfirePort];
	...
	// Start the keep-alive timer to expire every minute.
...
+ (NSDictionary *)defaultUserOptions
{
	return [NSDictionary dictionaryWithObjectsAndKeys:
		...
		[NSNumber numberWithBool:NO],   kXfireShowChatTimeStampsOption,
...
- (XfireChat *)chatForSessionID:(NSData *)anID
{
	...
	for( i = 0; i < len; i++ )
	{
		...
		if( [[ch sessionID] isEqualTo:anID] )
		{
			return ch;
		}
	}
...
+ (XfireSession *)newSessionWithHost:(NSHost *)host port:(unsigned short)portNumber
{
