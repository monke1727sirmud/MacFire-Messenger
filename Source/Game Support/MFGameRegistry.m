...
- (id)init
{
	...
	if( self )
	{
		...
		if( path == nil )
		{
			...
			NSLog(@"Game information file is missing");
			[self release];
			return nil;
		}
		...
		{
			// TODO: need to get some kind of warning up to the user
			NSLog(@"Could not load game information file");
			[self release];
			return nil;
		}
		
		// Then load the icons file
		path = [[NSBundle mainBundle] pathForResource:@"icons" ofType:@"mar"];
		if( path == nil )
		{
			// TODO: need to get some kind of warning up to the user
...
- (BOOL)loadIconsFile:(NSString *)path
{
	...
	@try
	{
		...
		for( i = 0; i < cnt; i++ )
		{
			...
			img = [[NSImage alloc] initWithData:[file data]];
			if( img )
			{
		...
		_defaultImage = [[NSImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"MacFire" ofType:@"icns"]];
